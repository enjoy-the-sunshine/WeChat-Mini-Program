...
Page({
  ...
  async showModal(event) {
    ...
    switch (item) {
      ...
      case '咖啡因摄入目标':
        this.setData({ currentItem: 'caffeineGoal', modalTitle: '请输入咖啡因摄入目标（mg）', modalType: '输入', modalVisible: true, modalInput: '' });
        return;

      case '睡前咖啡因水平':
        this.setData({ currentItem: 'caffeineLevelBeforeSleep', modalTitle: '请输入睡前咖啡因水平（mg）', modalType: '输入', modalVisible: true, modalInput: '' });
        return;
